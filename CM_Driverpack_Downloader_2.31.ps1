...
$ScriptVersion = "2.31 Gui (3/11/2020)"
...
if($oPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator )){   
    ...
    #Write-Warning "Script does not have Local Administrator rights!"
    $AdminRights = $false
}
...
Function ListView_FillInItem {
    ...
    if ( $pHPDriverpack -eq $null ) {
        $pModelsList[3,$pRow].value = $null                                    # Available
        $pModelsList[4,$pRow].value = $null                                    # CM Ver
        $pModelsList[5,$pRow].value = $null                                    # CM OS
        $pModelsList[6,$pRow].value = $null                                    # CM Package
        $pModelsList[7,$pRow].value = $null                                    # Step
    } else {
...
Function CM_Report {
    ...
    for ( $i = 0; $i -lt $pModelsList.RowCount; $i++ ) {
        if ( $i -in $pCheckedItemsList ) {
            ...
            CMTraceLog -Message "Checking: row:$($i); Model=/$($lEntryModel)/; ProdCod=/$($lEntryProdCode)" -Type $TypeDebug -LogFile $LogFile   
	        ...
	        if ($HPDriverPack) {
                ...
                if ($CMDriverPackage) {
                    ...
                    if ( $lOSVerMatch ) {
			            ...
			            CMTraceLog -Message "... CM Driver Package NOT Found, HP version: $($HPDriverPack.Version)" -Type $TypeError -LogFile $LogFile
		        ...
		        CMTraceLog -Message "... No HP Driver Pack Available for '$($lEntryModel)' Product Code '$($lEntryProdCode)'" -Type $TypeError -LogFile $LogFile
...
Function CM_Download {
    ...
    for ( $i = 0; $i -lt $pModelsList.RowCount; $i++ ) {
        if ( $i -in $pCheckedItemsList ) {
            ...
            CMTraceLog -Message "Checking: table row:$($i); Model=$($lEntryModel); ProdCod=$($lEntryProdCode)" -Type 1 -LogFile $LogFile 
...
Function Manage_CM_Folders {
    ...
    CMTraceLog -Message "Manage_CM_Folders!" -Type 1 -LogFile $LogFile
    Set-Location -Path "$($SiteCode):" | Out-Null
    ...
    Set-Location -Path "C:" | Out-Null
    CMTraceLog -Message "Done - Manage_CM_Folders!" -Type 1 -LogFile $LogFile
...
Function CreateForm {
    ...
    $ShowHelp={
        ...
        Switch ($this.name) {
            ...
            "Use_OS_Folders"     {$tip = "Put Driver Packages in CM folders based on OS version"}
        } # Switch ($this.name)
        ...
        if ( $Script:OSVER -in $Script:OSVALID ) {
            ...
            if ($Script:CMConnected -eq $true) {
                switch ( $ActionComboBox.SelectedItem ) {
                    'Report' { CM_Report $dataGridView $lCheckedListArray }
                    'Download' { CM_Download $dataGridView $lCheckedListArray }
                    'Setup CM Task Sequences' { CM_CheckTS }
    ...
    $OSTextLabel.location = New-Object System.Drawing.Point($LeftOffset,($TopOffset+36))    # (from left, from top)
    ...
    $OSVERComboBox.Location  = New-Object System.Drawing.Point(($LeftOffset+70), ($TopOffset+34))
    ...
    $dataGridView.Columns[7].Name = 'TS Step'
    $dataGridView.Columns[7].Width = 55
